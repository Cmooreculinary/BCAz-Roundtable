...
class TestMembersAndMessages:
    ...
    def test_get_members(self, auth_session):
        ...
        if data:
            ...
            print(
                f"Sample member: {member.get('name')}, avatar_url: {member.get('avatar_url', 'None')}"
            )
    ...
    def test_get_table_detail(self, auth_session):
        ...
        if tables:
            ...
            if data.get("members"):
                ...
                print(
                    f"Table member: {member.get('name')}, avatar_url: {member.get('avatar_url', 'None')}"
                )
...
class TestUserProfile:
    ...
    def test_update_profile(self, auth_session):
        ...
        response = auth_session.put(
            f"{BASE_URL}/api/me", json={"name": current.get("name", "Admin")}
        )
        assert response.status_code == 200
        ...
        if members:
            ...
            if other:
                response = auth_session.post(
                    f"{BASE_URL}/api/walkie/ping", json={"to_user": other.get("id")}
                )
