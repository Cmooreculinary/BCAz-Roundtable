...
function LiveTableView({ config, seated, simRunning, simStep, setSimStep, setSimRunning, setTab, autoSeat }) {
  ...
  useEffect(() => {
    ...
    const SIM_STEPS = [
      ...
      { delay: 3000, action: () => setChatMessages([{ from: "Chris", text: "Tonight we are looking at the launch path for Round Table and The Gather App.", color: "#007AFF" }]) },
      ...
      { delay: 3000, action: () => setAiSummary("Round Table demo complete. Key value: configurable rooms, table settings, seated avatars, ambiance, service layer, and group tools in one visual gathering space.") },
      { delay: 4000, action: () => {} },
    ];
  ...
  const SIM_LABELS = [
    ...
    "Round Table: Choose the room. Set the table. Seat the people. Shape the gathering.",
  ];
    ...
    <div style={{ position: "relative" }}>
      ...
      {!simRunning && simStep >= 11 && (
        <div style={{
          ...
          <h2 style={{ fontSize: 28, fontWeight: 800, margin: 0, letterSpacing: "-0.02em" }}>Round Table</h2>
...
function ExploreGrid() {
  return (
    <div>
      <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 16 }}>What Round Table Can Become</h2>
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, padding: 24, border: "1px solid rgba(255,255,255,0.08)", lineHeight: 1.8, fontSize: 14, color: "rgba(255,255,255,0.7)" }}>
        ...
        <p style={{ marginTop: 16, color: "#FFCC00", fontWeight: 600 }}>Round Table is where your people gather. This demo proves the vision is real.</p>
      </div>
    </div>
  );
}
